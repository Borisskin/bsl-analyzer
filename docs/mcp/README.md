...
### Целевой запуск по HTTP

Следующие команды описывают интерфейс после внедрения
[согласованного плана HTTP](../../plans/BSL_ANALYZER_MCP_HTTP_PORT_PLAN.md).
До завершения реализации и проверок их нельзя считать подтверждением
готовности текущего бинарника.
...
Оба сервера публикуют MCP по адресу
`http://127.0.0.1:<порт>/mcp`. Состояние процесса доступно по `/health`.
Путь `/mcp` не настраивается.
...
- целевой HTTP-сервер ограничивает размер тела запроса;
- `workspace` по-прежнему требует `--source-dir`;
- `reference` не принимает `--source-dir` и параметры подключения к 1С;
...
а TLS и проверку подлинности настройте на обратном посреднике. `/health` не
должен раскрывать путь проекта, параметры подключения к 1С или секреты.
